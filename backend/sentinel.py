...
def detect_cycles(graph):
    # Guard against RecursionError on large repos
    limit = max(1000, len(graph) * 3)
...
def analyze_architecture_debt(repo_path):
    ...
    # Include ALL source files as nodes — disconnected files get degree 0
    node_ids = [os.path.relpath(fp, repo_path).replace('\\', '/') for fp in source_files]
    node_index = {rel: i for i, rel in enumerate(node_ids)}
    ...
    for src, targets in path_graph.items():
        ...
        for tgt in targets:
            ...
            if tgt_idx is not None and tgt != src:
                edges.append({'source': src_idx, 'target': tgt_idx})
    ...
    return {
        ...
        'issues': all_issues,
        'summary': {
            'circular_dependencies': len(circular_issues),
            'god_files': len(god_issues),
            'oversized_files': len(large_issues)
        },
...
if __name__ == '__main__':
    ...
    if isinstance(arch_map, dict):
        nodes = arch_map.get('nodes', [])
        edges = arch_map.get('edges', [])
        print(f"\nArchitecture map: {len(nodes)} nodes, {len(edges)} edges")
